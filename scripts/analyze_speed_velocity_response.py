...
from data_analysis.speed_velocity_response import analyze_speed_velocity_response
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Analyze acceleration-derived velocity response by spindle speed.")
    parser.add_argument("--data-root", type=Path, default=Path("data/runs"), help="Run-directory root")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("analysis_out/velocity_response"),
        help="Output directory for CSV, JSON, PNG, Markdown, and velocity .npz.xz files",
    )
    parser.add_argument("--analysis-duration-s", type=float, default=60.0, help="Stable-window duration per run")
    parser.add_argument("--highpass-hz", type=float, default=10.0, help="Velocity integration high-pass cutoff")
    parser.add_argument(
        "--lowpass-hz",
        type=_optional_float,
        default=1000.0,
        help="Velocity integration low-pass cutoff; use 'none' to disable",
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        analysis = analyze_speed_velocity_response(
    ...
    print(f"Analyzed runs: {len(analysis['runs'])}")
