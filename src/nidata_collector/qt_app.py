...
DEFAULT_Y_RANGES = {
    SignalType.ACCELERATION: (-50.0, 50.0),
    SignalType.TEMPERATURE_RTD: (-50.0, 200.0),
}
...
class DataCollectorQtApp(QtWidgets.QMainWindow):
    def __init__(self, initial_refresh: bool = True) -> None:
        ...
        self.plot_y_ranges: dict[SignalType, tuple[float, float]] = dict(DEFAULT_Y_RANGES)
    ...
    def _build_vibration_settings(self, tabs: QtWidgets.QTabWidget) -> None:
        ...
        self.accel_settle = self._double_spin(0.0, 120, 0.5, decimals=3)
    ...
    def _build_temperature_settings(self, tabs: QtWidgets.QTabWidget) -> None:
        ...
        self.temp_rate = self._double_spin(0.001, 1000, 1, decimals=6)
        self.temp_samples = self._int_spin(1, 50_000_000, 10)
        self.temp_seconds = self._double_spin(0.001, 86400, 10, decimals=6)
        self.temp_min = self._double_spin(-273.15, 10000, -50, decimals=3)
        self.temp_max = self._double_spin(-273.15, 10000, 200, decimals=3)
        ...
        fields = [
            ...
            ("Excitation A", self.temp_excitation),
            ("R0 ohms", self.temp_r0),
            ("RTD type", self.temp_rtd_type),
            ("Wiring", self.temp_wire),
        ]
        self._add_form_fields(grid, fields)
        tabs.addTab(page, "RTD Temperature")
    ...
    def _build_plot_card(self) -> QtWidgets.QWidget:
        ...
        self.accel_plot_min = self._double_spin(-100000, 100000, -50, decimals=3)
        self.accel_plot_max = self._double_spin(-100000, 100000, 50, decimals=3)
        self.temp_plot_window = self._double_spin(1, 86400, 600, decimals=3)
        self.temp_plot_min = self._double_spin(-273.15, 10000, -50, decimals=3)
        self.temp_plot_max = self._double_spin(-273.15, 10000, 200, decimals=3)
        for spin in (
    ...
    def refresh_devices(self) -> None:
        ...
        try:
            ...
            QtWidgets.QMessageBox.critical(self, "DAQmx", f"Device refresh failed:\n{exc}")
            return
        ...
        if not self.channel_rows:
            empty = QtWidgets.QLabel("No supported NI 9234 or NI 9216 AI channels detected.")
    ...
    def start_acquisition(self) -> None:
        ...
        for panel in self.plot_panels.values():
            panel.clear()
        self.plot_dirty = set(SignalType)
        ...
        self.trigger_button.setEnabled(config.has_saves)
    ...
    def _refresh_plot_display_settings(self) -> None:
        try:
            ranges = {signal_type: self._plot_y_range_for(signal_type) for signal_type in SignalType}
        ...
        self.plot_dirty.update(SignalType)
    ...
    def _plot_y_range_for(self, signal_type: SignalType) -> tuple[float, float]:
        if signal_type == SignalType.ACCELERATION:
            ...
            label = "Vibration"
        elif signal_type == SignalType.TEMPERATURE_RTD:
    ...
    def _build_run_configuration(self) -> RunConfiguration:
        ...
        for signal_type in SignalType:
            ...
            if not channels:
                continue
            settings = self._acceleration_settings() if signal_type == SignalType.ACCELERATION else self._temperature_settings()
    ...
    def _handle_data_event(self, group: str, payload: dict) -> None:
        ...
        self.plot_dirty.add(signal_type)
    ...
    def closeEvent(self, event: QtCore.QEvent) -> None:
        ...
        for result in unreserve_network_devices(self.reserved_device_names):
...
class PlotPanel(QtWidgets.QWidget):
    ...
    def _ensure_plot(
        ...
        plot_widget.setTitle(channel, size="9pt")
        plot_widget.setMouseEnabled(x=False, y=False)
        plot_widget.setMenuEnabled(False)
        plot_widget.enableAutoRange(x=False, y=False)
        curve_a = plot_widget.plot(pen=pg.mkPen("#1f77b4", width=1))
        curve_b = plot_widget.plot(pen=pg.mkPen("#1f77b4", width=1))
